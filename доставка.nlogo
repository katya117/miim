...
breed [audience aud]
breed [turs tur]
audience-own [t free new]
turs-own [work time_work]
to setup
  clear-all
 setup-servers
  set time 0
  set r ((random 6) + 3)
  set people koldostavchik
  set time 0
set kol 0
  ask patches [set pcolor 62]
end
to setup-servers
  set-default-shape turs "telephone"
  create-ordered-turs dispetchers [
    set color 9.9
    set size 4
    set work 0
...
to go
    ...
    ask audience [
      set t (t - 1)
      if (t = 0) [
        ask my-out-links [ask other-end [ set work 0]]
        die
...
to available_turs
  ...
  ask turs with [work = 0] [
    let new_group audience with [free = 0]
